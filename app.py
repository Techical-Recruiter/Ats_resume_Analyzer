...
async def analyze_resume():
    try:
                ...
                try:
                    ...
                    if response_json:
                        ...
                        if has_must_have:
                            ...
                            good_to_have_score = sum(1 for keyword in good_to_have_list if keyword.lower() in resume_text) if good_to_have_list else 0
                            response_json["good_to_have_score"] = good_to_have_score
                            response_json["resume_index"] = idx
                            results.append(response_json)
                            st.write(f"Resume {idx} matched: {response_json['##JD Match']}, Good-to-have score: {good_to_have_score}")
                except Exception as e:
        ...
        if not results:
            ...
            results.sort(key=lambda x: (int(x["##JD Match"].replace("%", "")), x["good_to_have_score"]), reverse=True)
