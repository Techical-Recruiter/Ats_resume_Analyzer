...
if st.session_state.current_page == "role_selection":
    ...
    if st.button("Continue"):
        ...
        if user_role == "Job Seeker":
            ...
            st.session_state.page_history.append("recruiter_dashboard")
            st.session_state.cv_count = 0
            st.session_state.uploaded_files_hash.clear()
        st.rerun()
elif st.session_state.current_page == "job_seeker_dashboard":
    logged_in_user = st.session_state.logged_in_user
    st.title("CV Ranker")
    st.markdown("**Powered by <a href='https://PakistanRecruitment.com' target='_blank'>PakistanRecruitment</a>**", unsafe_allow_html=True)
    st.write(f"Welcome" if not logged_in_user else f"Welcome, {logged_in_user}!")
    ...
    st.markdown("**Powered by <a href='https://PakistanRecruitment.com' target='_blank'>PakistanRecruitment</a>**", unsafe_allow_html=True)
    st.write(f"Welcome" if not logged_in_user else f"Welcome, {logged_in_user}!")
    plan = st.session_state.USERS.get(logged_in_user, {}).get("plan", None) if logged_in_user else None
    ...
    resume_count = st.session_state.get(f"{logged_in_user}_recruiter_resumes_analyzed", 0) if logged_in_user else st.session_state.cv_count
    
    # Check limit before allowing upload
    if plan in ["free_recruiter", "basic"] and resume_count >= limit:
    ...
    # Handle uploads and enforce limit
    if upload_files:
        unique_files = []
        for upload_file in upload_files:
            file_hash = hashlib.md5(upload_file.getvalue()).hexdigest()
            if file_hash not in st.session_state.uploaded_files_hash:
                st.session_state.uploaded_files_hash.add(file_hash)
                unique_files.append(upload_file)
        if unique_files:
            current_cv_count = len(unique_files)
            if not logged_in_user:
                st.session_state.cv_count += current_cv_count
                if st.session_state.cv_count >= 10:
                    st.warning("You have reached the 10 resume limit. Please sign up to continue.")
                    upload_files = None
                    if st.button("Sign Up"):
                        st.session_state.current_page = "recruiter_signup"
                        st.session_state.page_history.append("recruiter_signup")
                ...
                st.session_state[f"{logged_in_user}_recruiter_resumes_analyzed"] += current_cv_count
                if st.session_state[f"{logged_in_user}_recruiter_resumes_analyzed"] >= limit:
                    st.warning(f"You have reached the {limit} resume limit on your current plan. Please upgrade to continue.")
                    upload_files = None
    ...
    async def analyze_resume_advanced():
        try:
            ...
            if plan in ["free_recruiter", "basic"] and st.session_state.get(f"{logged_in_user}_recruiter_resumes_analyzed", 0) + resume_count > limit:
                st.warning(f"You have reached the {limit} resume limit on your current plan. Please upgrade to continue.")
                return
            batch_size = 10
            ...
            if not results:
                ...
                if plan in ["free_recruiter", "basic"]:
                    st.session_state[f"{logged_in_user}_recruiter_resumes_analyzed"] = st.session_state.get(f"{logged_in_user}_recruiter_resumes_analyzed", 0) + resume_count
